  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-2xl border p-6 mb-6 flex flex-wrap gap-3">
        ...
        />
        <button onClick={add} className="bg-brand-ink text-white rounded-full px-5 py-2 flex items-center gap-2">
        ...
        {items.map((m) => (
          <div key={m.id} className="p-4 flex items-center gap-3">
            <input
              ...
              className="border rounded-full px-4 py-1.5 flex-1"
            />
            <input
              defaultValue={m.url}
            ...
            <input
              ...
              className="border rounded-full px-3 py-1.5 w-20"
            />
            <button
              onClick={() => upd(m.id, { aktif: !m.aktif })}
              className={`px-3 py-1 rounded-full text-xs ${m.aktif ? "bg-green-100 text-green-700" : "bg-gray-100"}`}
            >
              {m.aktif ? "Aktif" : "Pasif"}
            </button>
            <button onClick={() => del(m.id)} className="p-2 text-red-600 hover:bg-red-50 rounded">
